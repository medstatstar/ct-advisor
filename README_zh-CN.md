...
ct-advisor 是一个**对话式技能**：你只要把正在做的事告诉助手即可 —— 无需命令、无需记参数名。作为 WorkBuddy 技能，它**自动加载，无需额外安装**。
...
- **无密钥、本地优先**：顾问绝不泄露个人信息、受试者信息、未公开项目数据、私有路径或凭据。方法学运行零出站流量。
...
### 扫描器误报说明 部分自动化扫描器会标记 `adapters/`，因其含看似网络或凭据相关的字符串。**这些均为惰性桩，默认（本地）模式下从不执行** —— `CozeBackend._post()` 仅在 `config.json` 设 `backend: coze` 且配置了 `coze.bot_id` 时才被触及；`import requests` 为延迟导入；仓库内无密钥（Coze token 调用时从环境变量 `COZE_TOKEN` 读取）。按发布原样运行即保持零出站。

---

**版本**：v0.7.12 | **许可证**：MIT | **作者**：medstatstar, phoe-zip
