...
# -*- coding: utf-8 -*-
"""
bug_report.py — ct-advisor「技能错误报告」客户端 (ct-base §20.3；叶子技能定制版)

功能（复制到 ct-advisor/adapters/ 使用，§16.9 出站目录）：
...
设计约束：
  ...
  - 仅标准库；端点为统一 bug-report 端点（见 DEFAULT_ENDPOINT）。
  - 出站鉴权：send 前必须经技能既有出站授权闸门（§5）确认；本文件不自行授权。
  - query_origin（§8.6）：sha256(hostname)，随每个报告发送，服务端归因/限流。
  - 凭据（§5 公用凭据）：统一端点 token 以 XOR+base64 混淆内嵌于本文件（EMBEDDED_SECRETS），
    与 ct-base 镜像同密钥同 blob；可随包发布（公共凭据，非私有）。
"""
...
REPORT_SCHEMA = {
    "skill": str,            # 技能名（如 "ct-advisor"）
    "skill_version": str,    # 技能版本（如 "0.9.70"）
...
# ── 统一报告端点（§20.3.5 统一端点，作者预置公共端点，已发布 2026-08-21）──
# 全库共用一个端点，由作者一次性建设：接收 → 校验 → 落库 → 通知。
DEFAULT_ENDPOINT = "https://ct-bugreport.coze.site/run"  # ct-base §20.3.5 统一端点（作者预置公共端点）
AUTHOR_EMAIL = "medstatstar@gmail.com"  # §13.2 联系方式（本地兜底）

# ── 内嵌公共凭据（§5 公用凭据：XOR+base64 混淆，可随包发布）──────────────
# 叶子技能接入时按 ct-base §20.3.6 定制：内嵌统一 bug-report 端点 token。
# 与 ct-base 镜像（adapters/coze/src/endpoint_token.py）同密钥同 blob。
_EMBEDDED_SECRETS = {
    "ct_bugreport_coze": (
...
# token 解析优先级：CLI(--token) > env(CT_BUGREPORT_TOKEN) > 内嵌 blob；皆无回退空串。
def get_token(cli_token: str = None, token_env: str = "CT_BUGREPORT_TOKEN") -> str:
    if cli_token:
        return cli_token
    ...
    if env:
        return env
    try:
        return _obf_decode(_EMBEDDED_SECRETS["ct_bugreport_coze"])
    except Exception:
        return ""


_EMBEDDED_TOKEN = get_token()
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="ct-advisor bug-report client (ct-base §20.3)")
    p.add_argument("--skill", default="ct-advisor")
    p.add_argument("--skill-version", default="0.9.70")
    p.add_argument("--test", default="unknown")
    p.add_argument("--error-type", default="error")
    p.add_argument("--error-code", default="")
    p.add_argument("--engine-status", default="")
    p.add_argument("--description", default="")
    p.add_argument("--attempts", type=int, default=1)
    p.add_argument("--send", action="store_true",
                   help="actually POST the sanitized report (only after user confirmation)")
    p.add_argument("--demo", action="store_true",
                   help="template self-test (local md only, no network)")
    args = p.parse_args()
    if args.demo:
        ...
        rep = build_report(skill=args.skill, skill_version=args.skill_version,
                           test=args.test, error_type=args.error_type,
                           error_code=args.error_code, engine_status=args.engine_status,
                           description=args.description, attempts=args.attempts)
        if args.send:
            res = send_to_endpoint(rep)
            print(json.dumps(res, ensure_ascii=False))
        else:
            print(confirm_prompt(error_type=args.error_type, test=args.test,
                                 locale=_current_locale()))
            print(render_report_text(rep))
            print("(add --send after the user confirms to POST the sanitized report)")
